...
export default function OrderedQuantityTabOption() {
  ...
  useEffect(() => {
    let urlapi = "api/CustomerOrders/ordered-quantity-report-data?";
    let urlPDFlink = "/api/CustomerOrders/ordered-quantity-report?";
    let urlEXCELlink = "/api/CustomerOrders/ordered-quantity-report-excel?";
  ...
  const rows = data?.map((elem) => {
    return {
      ...
      DateOfProductRequired: formatDate(elem.date),
    };
  });
  ...
  const headCells = [
    ...
    {
      id: "DateOfProductRequired",
      numeric: true,
      disablePadding: false,
      label: "Date Of Product Required",
    },
  ];
  return (
    <>
      <HeaderFilter
        ...
        onPrint={() =>
          handlePrint(
            urlPDF,
            `Ordered-Quantity-report-${formatDate(new Date())}.pdf`,
            auth
          )
        }
        export={true}
        ...
        onExport={() =>
          handleExcel(
            urlEXCEL,
            `Ordered-Quantity-excel-report-${formatDate(new Date())}.xlsx`,
            auth
          )
        }
      />
