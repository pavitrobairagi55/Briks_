...
export default function InventoryBinCardsTabOption() {
  ...
  const { data: subItems } = useFetch("SubItems?isFinished=true");
  ...
  useEffect(() => {
    let urlapi = `api/Voucher/inventory-bin-cards-report-data?ProductId=${selectValue}`;
    let urlPDFlink = `/api/Voucher/inventory-bin-cards-report?ProductId=${selectValue}`;
    let urlEXCELlink = `/api/Voucher/inventory-bin-cards-report-excel?ProductId=${selectValue}`;
    ...
    <>
      <HeaderFilter
        ...
        onPrint={() =>
          handlePrint(
            urlPDF,
            `Bin-Card-Inventory-report-${formatDate(new Date())}.pdf`,
            auth
          )
        }
        export={true}
        ...
        onExport={() =>
          handleExcel(
            urlEXCEL,
            `Bin-Card-Inventory-excel-report-${formatDate(new Date())}.xlsx`,
            auth
          )
        }
      />
