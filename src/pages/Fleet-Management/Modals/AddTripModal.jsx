...
export default function AddTripModal({
  ...
  const [date, setDate] = useState(convertExistingDate(data.productionDate));
  ...
  const [subItemId, setSubItemId] = useState(data?.product?.id);
  ...
  useEffect(() => {
    if (loadQuantity > data.remainingQuantity) {
      setError("Load Quantity can't be more than remaining quantity");
    } else {
      setError(null);
    }
  }, [loadQuantity]);
  ...
  const saveData = async () => {
    ...
    try {
      ...
      await axios.post("api/Trips", input, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${auth.token}`,
        },
